...
const delay = process.env.DELAY != null ? Number(process.env.DELAY) : 2000;
...
async function publisher() {
  ...
  const sendMessage = async (connection, channel, message) => {
    const sent = channel.publish(
      ...
      {
        //depende de si es necesario persisitir el mensjae.
        //ya que tiene mas proceso al hacer commits al disco
        persistent: true,
      }
    );
